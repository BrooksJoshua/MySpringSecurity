...
public class FutureDemo {
    public static void main(String[] args) throws Exception {
        ...
        //Integer v = f.get(1, TimeUnit.SECONDS);//由于等待时间过短,任务来不及执行完成,会报异常 java.util.concurrent.TimeoutException
        //System.out.println("任务执行的结果是:"+v);
    }
    //正常测试流程
    private static void test1(Future<Integer> f) throws InterruptedException, ExecutionException, TimeoutException {
        boolean done = f.isDone();
        System.out.println("第一次判断任务是否完成:"+done);
        boolean cancelled = f.isCancelled();
        System.out.println("第一次判断任务是否取消:"+cancelled);
        Integer v = f.get(1,TimeUnit.SECONDS);//一直等待任务的执行,直到完成为止
...
class MyCall implements Callable<Integer>{
    ...
    public Integer call() throws Exception {
        ...
        Thread.sleep(2000);// 模拟业务执行流程所需时间为2秒
